...
The original ground-truth tool was a Windows desktop app: one annotator at a time, tied to Active Directory and a company VPN, slow enough that throughput on a 5k image batch was a real bottleneck, and the controls through WinForms just didn't feel good at all. WebGLGT removes those constraints. There's no AD, no VPN, no SSO dependency — anyone with a URL and credentials can annotate, which makes it usable by temp staff and outside contractors. It also keeps the keyboard-driven workflow the existing annotators relied on, so adopting it didn't mean retraining anyone while also adding smooth intuitive new control methods to speed up processing.
...
- **Compositing the OCR payload on the GPU.** As mentioned previously with the draw-region stitching, the naive design is one Vision call per selection group, which gets expensive and slow fast. Doing the layout as boolean mesh operations rather than CPU-side image stitching was the unlock — and it keeps the whole crop/stitch step on the same canvas the annotator is already looking at.
